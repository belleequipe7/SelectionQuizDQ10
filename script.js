...
class QuizGame {
    constructor() {
        ...
        this.versionInfoContainer = document.getElementById('version-info-container');
        this.setVersionInfo();
    ...
    setVersionInfo() {
        this.versionInfoContainer.innerHTML = `
            <p>ver.1.0.0</p>
            <p>production by M.F. (Frieren:ID )</p>
        `;
    }

    bindEvents() {
        document.getElementById('start-v1-btn').addEventListener('click', () => this.startGame(1));
        document.getElementById('start-v2-btn').addEventListener('click', () => this.startGame(2));
        document.getElementById('restart-btn').addEventListener('click', () => {
            this.resultPopup.classList.add('hidden');
            // Helper to get back to start screen or restart same version?
            // The user request says "return to initial screen" usually for exit, but restart might imply same quiz.
            // Let's make "Play Again" go back to start screen so they can choose version again or restart same.
            // actually, typical quiz behavior is restart same settings. But let's check what's easiest.
            // The existing `returnToStart` does what we want for "Exit".
            // For "Restart", we need to know which version was played.
            // Let's store currentVersion.
            this.startGame(this.currentVersion);
        });
        ...
        // Title Click Event - Changed to Subtitle
        this.gameSubtitle.addEventListener('click', () => this.showPlayCount());
    }
    ...
    startGame(version) {
        ...
        // Filter Questions by Version
        const versionQuestions = allQuestions.filter(q => q.version === version);

        if (versionQuestions.length === 0) {
            alert("No questions found for this version!");
            return;
        }

        // Select 10 random questions
        const shuffled = this.shuffleArray([...versionQuestions]);
    ...
    async endGame() {
        ...
        if (this.score === this.currentQuestions.length) {
            this.resultTitle.textContent = "全問正解！";
            this.resultMessage.innerHTML = `おめでとうございます！DQ10マスターです！<br>タイム: ${duration.toFixed(1)}秒${rankMsg}`;
            this.audio.playLevelUp();
        } else {
            this.resultTitle.textContent = "ゲーム終了";
            this.resultMessage.innerHTML = `あなたのスコアは ${this.score} / ${this.currentQuestions.length} です<br>タイム: ${duration.toFixed(1)}秒${rankMsg}`;
